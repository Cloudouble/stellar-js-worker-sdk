...
export default {
    ...
    createTransactionSourceObject: async function (transaction) {
        if (!transaction.memo) delete transaction.memo
        transaction.memo &&= typeof transaction.memo === 'string' ? { type: 'MEMO_TEXT', content: transaction.memo }
            : (Number.isInteger(transaction.memo) ? { type: 'MEMO_ID', content: transaction.memo } : transaction.memo)
        const tx = {
